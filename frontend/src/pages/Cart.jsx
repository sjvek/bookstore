...
function Cart() {
  ...
  const deletItem = async (bookid) => {
    try {
      ...
      toast.success(response.data.message);  // Success toast on item deletion
      setCart(Cart.filter((item) => item._id !== bookid));
    } catch (error) {
      toast.error("Failed to remove item from cart!");  // Error toast on failure
    }
  };
  ...
  const PlaceOrder = async () => {
    try {
      ...
      toast.success(response.data.message);  // Success toast on placing order
      navigate("/profile/orderHistory");
    } catch (error) {
      toast.error("Failed to place your order!");  // Error toast on failure
    }
  };
  ...
  return (
    <div className="bg-dark p-5 vh-100">
      ...
      {Cart && Cart.length === 0 && (
        <div className="d-flex flex-column align-items-center justify-content-center">
          <h1 className="fw-bold text-light">Empty Cart</h1>
          <p className="text-light-emphasis">Looks like you haven't added anything yet.</p>
        </div>
      )}
        ...
        <div>
          ...
          {Cart.map((items, i) => (
            ...
            >
              ...
              <div className="w-100 mx-3 mx-md-5">
                <h4 className="text-light">{items.title}</h4>
                <p className="text-light d-none d-md-block text-truncate" style={{ width: "550px" }}>
                  {items.desc}
                </p>
              </div>
      ...
      {Cart && Cart.length > 0 && (
        <div className="w-100 d-flex align-items-center justify-content-end text-light">
          <div className="p-3 rounded-3" style={{ backgroundColor: "#171717" }}>
            <h2>Total Amount</h2>
            <div className="d-flex justify-content-between">
              <h5>{Cart.length} books</h5>
              <h5>{Total}</h5>
